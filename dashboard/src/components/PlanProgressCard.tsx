...
export function PlanProgressCard({
  ...
  const dotClass = isDone ? 'bg-emerald-400/60' : 'bg-cyan-400/60';

  return (
    <div className={cn('rounded-md border border-fg-6 bg-panel overflow-hidden shadow-sm', className)}>
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-left hover:bg-white/[0.02] transition-colors"
      >
        <span className={cn('h-[7px] w-[7px] rounded-full shrink-0', dotClass)} />
        <span className="text-[10.5px] font-semibold uppercase tracking-[0.14em] text-fg-5">
          {progressLabel}
        </span>
        {plan.explanation && (
          <span className="flex-1 min-w-0 text-[11.5px] text-fg-4 truncate">{plan.explanation}</span>
        )}
        {!plan.explanation && <span className="flex-1" />}
        <span className={cn(
          'rounded border px-1.5 py-0.5 text-[10px] font-mono',
          isDone ? 'border-ok/20 text-ok' : 'border-fg-6 text-fg-5',
        )}>
      ...
      {open && (
        <div className="border-t border-fg-6 px-3.5 py-2.5 space-y-1">
